...
class Pizzeria:
    ...
    def get_price(self,pizza):
        ...
        return base_price, toppings_price, total_price

    def get_receipt(self,pizza):
        base_price, toppings_price, total_price = self.get_price(pizza)
        ...
        for topping in pizza.get_toppings():
            print(f"  - {topping}")
        print(f"\nYou ordered a {pizza.get_size()}\" pizza for ${base_price:.2f}")
...
while True:
    ...
    if action == "exit":
        ...
        pizza = pizzeria.place_order()
        pizzeria.get_receipt(pizza)
